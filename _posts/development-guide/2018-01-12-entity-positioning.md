...
场景中的所有实体必须在场景边界内，因为边界之外是由其他用户拥有的土地。

运行场景预览时，所有突出到场景外部的实体将被涂成红色，并且会删除它的碰撞。 当部署到 Decentraland 时，引擎将根本不会渲染这些在土地外的任何实体。

引擎会对场景中移动的实体位置进行不断检查，如果实体离开场景然后返回，它将被移除，然后再次正常渲染。

场景地面上的网格显示了场景的边界限制，默认情况下，_x_ 和 _z_ 轴的范围从 0 到 16，在 _y_ 轴上的范围从 20 到20。 您可以自由地将实体放置在 _y_ 轴上 0 以下的地下。

> 提示：如果您的场景需要更多地块，则可以将其添加到项目的 `scene.json` 文件中。 有关说明，请参见[场景元数据]({{ site.baseurl }}{% post_url /development-guide/2018-02-26-scene-metadata %})。 添加后，就能看到网格扩展到添加了的土地上。

重要的是要注意，*整个* 3D 模型必须在场景的范围内。 包括模型的 *边界框*。 一些 3D 模型的边界框可能大幅超出了网格本身，有时很难分辨何时发生这种情况。 当实体超出场景的边界时，在预览中，您将看到一个标记这些边界框的立方体。 整个立方体必须在您的场景范围内。

![](/images/media/bounding-box.png)

如果实体的立方体超出其网格的形状，则可能需要在外部编辑器中编辑 3D 模型减少边距，或修改模型中网格的旋转和缩放。
