...
`video` 实体需要在 `src` 中选择一个视频，可以是本地文件，也可以是指向远程视频的 URL。
...
- 要在 *glTF模型* 中启用碰撞，您可以：
  ...
  - 在像 Blender 这样的外部工具中编辑它们以包含 *collider mesh 碰撞网格*。
碰撞网格必须命名为 _x_collider_，其中 _x_ 是模型的名称。 因此，对于名为 _house_ 的模型，碰撞网格必须命名为 _house_collider_。
...
>提示：要查看场景中所有碰撞网格的限制，请使用 `dcl start` 启动场景预览，然后单击 `c`。 碰撞区域会用蓝线显示。
